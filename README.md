...
一個以三層式架構（Web Server + Application Server + 關聯式資料庫）實作的線上圖書借閱系統，提供使用者註冊、登入、瀏覽書籍、借書、還書與個人資料管理等功能。
...
- **展示層（controller）**：AuthController、BookController、BorrowController、BranchController、ProfileController，提供 RESTful API。
- **業務層（service）**：AuthService、BookService、BorrowService，處理業務邏輯（如密碼加密）。
- **資料層（repository）**：UserRepository、BookRepository、BorrowRepository、BranchRepository，透過 SimpleJdbcCall 呼叫 Stored Procedure。
- **共用層（common）**：DTO、設定（Security/CORS）、JWT 工具、安全過濾器。

## 主要功能

- 使用者註冊（手機號碼、密碼，並可填寫 email、地址、生日、預設取書館）
- 登入驗證（JWT，僅登入者可借還書與管理個人資料）
- 書籍列表瀏覽（公開）、依分類篩選、點擊展開書目詳情
- 借書、還書（以 Transaction 確保資料一致）
- 我的借閱清單
- 個人資料查詢與修改（email、地址、預設取書館可改；手機、生日不可改）
- 分館（取書館）資料表與查詢
...
├── frontend/         # Vue 前端
│   └── src/
│       ├── views/          # 頁面（登入/註冊/書單/我的借閱/個人資料）
│       ├── stores/         # Pinia 狀態管理
│       ├── router/         # 路由
│       └── api/            # axios 設定
└── DB/               # 資料庫腳本
    ├── DDL.sql             # 建立資料庫、資料表、Stored Procedure
    └── DML.sql             # 測試資料（書籍、庫存、分館）
```
...
啟動 MySQL 後，依序執行 DB/ 資料夾內的腳本（可用 MySQL Workbench 或指令）：

```
mysql -u root -p < DB/DDL.sql   # 建立資料庫、資料表、Stored Procedure
mysql -u root -p < DB/DML.sql   # 匯入測試資料（書籍、庫存、分館）
```
...
| user_profile | 使用者資料（與 user_account 為 1:1，含預設取書館外鍵） |
| books | 書籍書目（紙本與電子書共用） |
...
| sp_list_books | 查詢書籍清單（含可借數量與書目詳情） |
| sp_borrow_book | 借書（Transaction 改狀態 + 新增紀錄） |
| sp_return_book | 還書（Transaction 更新歸還時間 + 改狀態） |
| sp_list_my_borrows | 查詢個人未歸還借閱清單 |
| sp_list_branches | 查詢分館清單 |
| sp_get_my_profile | 查詢個人資料（含分館名稱） |
| sp_update_my_profile | 更新個人資料（email、地址、預設取書館） |

## 安全性設計

- **密碼保護**：使用 BCrypt 進行加鹽（salt）雜湊後儲存，資料庫不存明碼。
- **身分驗證**：採用 JWT。借還書、個人資料等受保護的 API 必須攜帶有效 token；瀏覽書目與分館清單則開放公開存取。
- **防 SQL Injection**：所有資料庫存取皆透過 Stored Procedure 並以參數化方式傳入，避免字串拼接。
- **防 XSS**：前端輸入經格式驗證，後端設定 Content-Security-Policy 等安全標頭；Vue 樣板預設對輸出內容進行 HTML 逸出（escape）。
